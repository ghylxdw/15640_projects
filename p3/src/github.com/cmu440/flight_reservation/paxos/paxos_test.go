...
func TestOld(t *testing.T) {
	...
	pxa[1] = MakeTest(pxh, 1, nil)
	pxa[2] = MakeTest(pxh, 2, nil)
	pxa[3] = MakeTest(pxh, 3, nil)
	pxa[1].Start(1, 111)

	waitmajority(t, pxa, 1)

	pxa[0] = MakeTest(pxh, 0, nil)
	pxa[0].Start(1, 222)

	waitn(t, pxa, 1, 4)

	if false {
		pxa[4] = MakeTest(pxh, 4, nil)
		waitn(t, pxa, 1, npaxos)
	}
